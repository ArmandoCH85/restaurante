                                        ...
                                        Forms\Components\Repeater::make('payment_methods')
                                            ->schema([
                                                Forms\Components\Grid::make(3)->extraAttributes(['class'=>'gap-3'])->schema([
                                                    ...
                                                    Forms\Components\Placeholder::make('remaining')->label('Estado')->content(function (Get $get){ $payments=$get('../../payment_methods')??[]; $totalPaid=0; foreach($payments as $p){ if(isset($p['amount']) && is_numeric($p['amount'])) $totalPaid+=(float)$p['amount']; } $remaining=(float)$this->total - $totalPaid; if($remaining>0){return new \Illuminate\Support\HtmlString("<div class='p-1 bg-orange-50 border border-orange-200 rounded text-center'><span class='text-orange-700 font-bold text-[11px]'>Falta: S/ ".number_format($remaining,2)."</span></div>");} elseif($remaining<0){return new \Illuminate\Support\HtmlString("<div class='p-1 bg-red-50 border border-red-200 rounded text-center'><span class='text-red-700 font-bold text-[11px]'>Exceso: S/ ".number_format(abs($remaining),2)."</span></div>");} return new \Illuminate\Support\HtmlString("<div class='p-1 bg-green-50 border border-green-200 rounded text-center'><span class='text-green-700 font-bold text-[11px]'>Completo ✓</span></div>"); })->live()->columnSpan(1),
                                                ]),
