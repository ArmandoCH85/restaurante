    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
        ...
        <div id="cancelDeliveryModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
                ...
                <form id="cancelDeliveryForm" action="{{ route('delivery.update-status', $deliveryOrder->id) }}" method="POST" class="mb-4">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="status" value="cancelled">
                    <textarea name="reason" id="cancelReason" class="w-full px-3 py-2 text-gray-700 dark:text-gray-300 border rounded-lg focus:outline-none focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600" rows="3" placeholder="Motivo de cancelación"></textarea>
                </form>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button onclick="document.getElementById('cancelDeliveryForm').submit()" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors duration-200">Confirmar</button>
                </div>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-8">
                ...
                @if($deliveryOrder->status === 'assigned' && $isAssignedDeliveryPerson)
                    <form action="{{ route('delivery.update-status', $deliveryOrder->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="status" value="in_transit">
                        <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors duration-200 flex items-center justify-center">
                ...
                @elseif($deliveryOrder->status === 'in_transit' && $isAssignedDeliveryPerson)
                    <form action="{{ route('delivery.update-status', $deliveryOrder->id) }}" method="POST">
                        ...
                        <button type="submit" class="w-full sm:w-auto px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors duration-200 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if(!in_array($deliveryOrder->status, ['delivered', 'cancelled']) && $isAssignedDeliveryPerson)
                    <button onclick="openCancelModal()" class="w-full sm:w-auto px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors duration-200 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
@if(session('error'))
    <div class="fixed bottom-4 right-4 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg" id="error-notification">
        {{ session('error') }}
        <button type="button" class="ml-2" onclick="document.getElementById('error-notification').style.display='none'">&times;</button>
    </div>
    <script>
        setTimeout(function() {
...
@if(session('success'))
    <div class="fixed bottom-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg" id="success-notification">
        {{ session('success') }}
        <button type="button" class="ml-2" onclick="document.getElementById('success-notification').style.display='none'">&times;</button>
    </div>
    <script>
        setTimeout(function() {
