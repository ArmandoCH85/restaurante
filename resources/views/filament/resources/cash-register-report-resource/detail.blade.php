<x-filament::page>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <x-filament::card class="bg-white rounded-lg shadow">
            <div class="p-4">
        ...
        <x-filament::card class="bg-white rounded-lg shadow">
            <div class="p-4">
                ...
                <p class="text-2xl font-bold text-gray-900">S/ {{ number_format($movements->sum('amount'), 2) }}</p>
            </div>
        </x-filament::card>
        ...
        <x-filament::card class="bg-white rounded-lg shadow">
            <div class="p-4">
                ...
                <p class="text-2xl font-bold text-gray-900">S/ {{ number_format($orders->sum('total'), 2) }}</p>
            </div>
        </x-filament::card>
    </div>
                            ...
                            @forelse($orders as $order)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('filament.admin.resources.reportes.dashboard-ventas.view', ['record' => $order->id]) }}" 
    ...
    <div class="mt-6 pt-4 border-t border-gray-200 flex justify-end space-x-3">
        @if(!$record->closing_datetime && auth()->user()->can('update', $record))
            <x-filament::button wire:click="$dispatch('close-modal', { id: '{{ $this->getId() }}' })">
                Cerrar Caja
            </x-filament::button>
        @endif
        <x-filament::button
            tag="a"
            href="{{ route('cash.register.reports.export.pdf', ['cashRegister' => $record->id]) }}"
            color="gray"
