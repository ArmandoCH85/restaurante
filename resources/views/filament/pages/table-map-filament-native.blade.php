<x-filament-panels::page>
    ...
    <x-filament::section class="mb-6">
        <x-slot name="headerActions">
            ...
            <div class="flex gap-2">
                ...
                <x-filament::button
                    x-on:click="$dispatch('open-modal', { id: 'filters-modal' })"
                    color="gray"
                    size="sm"
                    icon="heroicon-m-adjustments-horizontal"
                    :badge="$statusFilter || $locationFilter || $searchQuery ? collect([$statusFilter, $locationFilter, $searchQuery])->filter()->count() : null"
                    badge-color="primary"
                >
                    Filtros
                </x-filament::button>
    ...
    <x-filament::section>
        ...
        @if($viewMode === 'grid')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-4" wire:poll.5s>
                @forelse($tables as $table)
                    <div
                        wire:click="openPOS({{ $table->id }})"
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 p-6">
                @forelse($tables as $table)
                    ...
                    >
                        ...
                        <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent transform -skew-x-12 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>

                        {{-- Número de mesa principal --}}
                        <div class="relative z-10 {{ $mapStyles['text'] }} font-black text-3xl group-hover:scale-110 transition-transform duration-300">
                            {{ $table->number ?? str_replace('Mesa ', '', $table->name) }}
                        </div>

                        {{-- Capacidad con icono --}}
                        <div class="relative z-10 flex items-center gap-1 {{ $mapStyles['text'] }} text-sm font-semibold mt-2 opacity-80 group-hover:opacity-100 transition-opacity duration-300">
                            <x-heroicon-m-user-group class="h-4 w-4" />
                            <span>{{ $table->capacity }}p</span>
                        </div>

                        {{-- Indicador de estado en la esquina --}}
                        <div class="absolute top-2 right-2 w-3 h-3 rounded-full {{ match($table->status) {
                        ...
                        <div class="absolute inset-x-0 bottom-0 bg-black/80 text-white text-xs py-1 px-2 rounded-b-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-center">
                            {{ match($table->status) {
                                'available' => 'Disponible',
                                'occupied' => 'Ocupada',
                                'reserved' => 'Reservada',
                ...
                @empty
                    <div class="col-span-full">
                        <div class="text-center py-12">
                            ...
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No se encontraron mesas</h3>
    ...
    <x-filament::modal
        ...
        description="Personaliza la vista del mapa de mesas"
        width="2xl"
    >
        <div class="space-y-6">
            <div class="space-y-4">
            ...
            @php
                $activeFilters = collect([$statusFilter, $locationFilter, $searchQuery])->filter()->count();
            @endphp

            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <x-slot name="footerActions">
            ...
            <x-filament::button
                x-on:click="$dispatch('close-modal', { id: 'filters-modal' })"
                size="sm"
            >
                Aplicar Filtros
            </x-filament::button>
